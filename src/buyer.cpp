...
float get_customer_money_input() {
    ...
    while (!valid_input) {
        ...
        } else if (!is_valid_money_input(money)) {
            clean_screen();
...
void add_to_basket() {
    ...
    cin.ignore();
    cin.getline(product_name, MAX_NAME_LENGTH);
    ...
    if (quantity > g_shop.products[index].quantity) {
        cout << "Недостаточно товара в магазине!\n";
        pause_screen();
        return;
    }
    
    if (g_customer.total_items + quantity > g_customer.capacity) {
